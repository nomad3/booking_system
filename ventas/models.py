...
class Proveedor(models.Model):
    ...
    telefono = models.CharField(max_length=20, null=True, blank=True)  # Added field for phone number
...
class Producto(models.Model):
    ...
    def obtener_precio_actual(self, fecha_reserva=None):
        # Logic for dynamic pricing based on date or reservation
        return self.precio_base

class Cliente(models.Model):
    nombre = models.CharField(max_length=255)
    email = models.EmailField()
    telefono = models.CharField(max_length=20, null=True, blank=True)  # Added telefono field
...
class VentaReserva(models.Model):
    ...
    def __str__(self):
        return f"Venta/Reserva #{self.id} de {self.cliente.nombre}"
...
class Pago(models.Model):
    ...
    metodo_pago = models.CharField(max_length=50, choices=[('efectivo', 'Efectivo'), ('tarjeta', 'Tarjeta de Crédito/Débito'), ('transferencia', 'Transferencia Bancaria'), ('paypal', 'PayPal')])
