...
class Producto(models.Model):
    ...
    def obtener_precio_actual(self, fecha_reserva=None):
        ...
        reglas = reglas.filter(
            models.Q(fecha_inicio__isnull=True) | models.Q(fecha_inicio__lte=fecha_reserva),
            models.Q(fecha_fin__isnull=True) | models.Q(fecha_fin__gte=fecha_reserva),
        ).order_by('-prioridad')
...
class VentaReserva(models.Model):
    ...
    def calcular_total(self):
        ...
        for reserva_producto in self.reservaprodutos.all():
            # Obtener el precio dinámico si es un producto reservable
            if reserva_producto.producto.es_reservable:
                precio = reserva_producto.producto.obtener_precio_actual(self.fecha_reserva)
            else:
                precio = reserva_producto.producto.precio_base  # Para productos físicos
